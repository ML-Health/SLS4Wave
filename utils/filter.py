...
def filter(data):
    ...
    ecg_data_1 = np.array([filter_ecg_channel(x[0, :]) for x in data])
    output_samples.append(torch.from_numpy(ecg_data_1.copy()).float())
    del ecg_data_1

    ecg_data_2 = np.array([filter_ecg_channel(x[1, :]) for x in data])
    output_samples.append(torch.from_numpy(ecg_data_2.copy()).float())
    del ecg_data_2

    pleth_data = np.array([filter_ppg_channel(x[2, :]) for x in data])
    output_samples.append(torch.from_numpy(pleth_data.copy()).float())
    del pleth_data

    abp_data = np.array([filter_abp_channel(x[3, :]) for x in data.numpy()])
